...
@interface SCNavTabBar () <SCPopViewDelegate>
{
    ...
    UIImageView     *_arrowButton;          // arrow button旋转的Button
    
    UIView          *_line;                 // underscore show which item selected
    UIView          *_Downline;             //最下边的线
...
- (void)viewConfig
{
    CGFloat functionButtonX = _showArrowButton ? (SCREEN_WIDTH - 50) : SCREEN_WIDTH;
    if (_showArrowButton)
    {
        _arrowButton = [[UIImageView alloc] initWithFrame:CGRectMake(functionButtonX, 20, 50,50)];
        _arrowButton.layer.shadowColor = [UIColor groupTableViewBackgroundColor].CGColor;
        _arrowButton.image = _arrowImage;
        _arrowButton.userInteractionEnabled = YES;
        
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(functionButtonX , ARROW_BUTTON_WIDTH - 1, ARROW_BUTTON_WIDTH, 1)];
    ...
    _navgationTabBar = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE, functionButtonX, 90)];
...
- (void)showLineWithButtonWidth:(CGFloat)width
{
    ...
    _Downline = [[UIView alloc]initWithFrame:CGRectMake(0, NAV_TAB_BAR_HEIGHT - 1, 640, 1)];
    _Downline.backgroundColor = [UIColor lightGrayColor];
    [_navgationTabBar addSubview:_Downline];
}
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    {
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(buttonX+5 , 10, 50, 50)];
        imageView.image = [UIImage imageNamed:@"Persion_Image"];
        imageView.layer.cornerRadius = 25;
        imageView.layer.masksToBounds = YES;
        [_navgationTabBar addSubview:imageView];
        
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.titleLabel.font = [UIFont systemFontOfSize:15.0];
        button.frame = CGRectMake(buttonX, 60,60, 30);
